...
Die Netzlast aus dem Datensatz verbrauch weicht mit 458 TWh geringfügig von der summierten Stromerzeugung aus dem Datensatz erzeugung in Höhe von 448 TWh ab. Dadurch weichen die berechneten Teile geringfügig voneinander ab.

#### Berechnung
**To Do: Erklären, was hier gerechnet wird.**
...
## Verarbeitung: Die Berechnungen werden für einen kleiner dimensionerten Speicher wiederholt, default = [0.9, 0.8, 0.7, 0.6, 0.5]
...
def anteil_ee_berechnen(data, netzlast = -1, neue_speichergrößen = [0.9, 0.8, 0.7, 0.6, 0.5], output = False):
  ...
  if output: # Anteil EE ausgeben
    ...
    print(f"Summe Netzlast: {netzlast.sum()}\nSumme positive Restlast: {data.clip(lower = 0).sum()}\tSumme negative Restlast: {data.clip(upper = 0).sum()}\nSpeichergröße: {speichergröße0:.2f}\tAnteil EE ohne Speicher: {1 - ( data.clip(lower = 0).sum() / netzlast.sum() ) if len(netzlast) > 1 else '--':.2f}\n")
...
**Das ist ein unspektakuläres Ergebnis.**  
Das liegt daran, dass die einzuspeichernde Restlast mit 1.002.976 MWh, also rund 1 TWh, erheblich kleiner als die zu deckende positive Restlast mit 207.842.228, also rund 208 TWh, ist. Die durch Einspeicherung maximal verschiebbare erneuerbare Überschussproduktion entspricht weniger als einem halben Prozent der zu deckenden Restlast.

Anschließend Ausbauziele durchrechnen: welcher Anteil EE wird erreicht. *Querverweis auf die Ausbaufaktoren*  
Hinweis: Der künftige Stromverbrauch entspricht nicht dem aus dem Jahr 2023, die Berechnungen dienen der Veranschaulichung. Das kann durchaus eine Aufgabe werden:  
**Rechnen Sie die Anteile auf der Grundlage der Stromverbrauchprojektion 2030 aus.**  
