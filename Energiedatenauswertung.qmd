...
## Mögliche Erweiterung / mögliche Aufgabe
... wäre gar nicht so schlecht, weil damit der Strommix im Pumpspeicher bestimmt würde ...
...
# Analytische Datenanalyse
Analytische Datenanalyse bedeutet, auf Grundlage der Daten Rückschlüsse zu ziehen. Die analytische Datenanalyse unterscheidet sich von der beschreibenden und explorativen Datenanalyse dadurch, dass Daten nicht nur betrachtet und zueinander ins Verhältnis gesetzt werden, sondern neue Daten erzeugt werden.
...
**Dazu muss auf eine Listenoperation zurückgegriffen werden. Ich kann bei Bedarf auch die Probleme mit NumPy / Pandas erklären (pd.Series.cumsum.clip(0) maskiert nur die Ausgabe bzw. fehlt pd.Series.cumsum() ein Argument upper_/lower_boundary).**
...
Nachdem die Speichergröße bestimmt wurde, kann der Jahresgang des Speichers für das erste und das zweite Jahr bestimmt werden. Der Jahresgang des Speichers für das zweite Jahr ist das stabile Gleichgewicht, mit dem eine gegebene Restlastkurve beliebig oft durchlaufen werden kann.

  - Eingabe: Der folgenden Funktion kann eine Speichergröße übergeben werden, andernfalls wird diese durch Aufruf der Funktion berechne_speichergröße() ermittelt.
  ...
  - Ausgabe: Die Funktion gibt, wenn `output = False` ist, ein Tupel zurück. An Position 0 ist die Speichergröße gespeichert, an Position 1 der Jahresgang in Jahr 1 als pd.Series, an Position 2 der Jahresgang in Jahr 2 als pd.Series. Die Jahresgänge werden im Folgenden Panel grafisch dargestellt.
...
**To Do: fig-alt**  
**eigentlich ist die Ausspeicherung falsch dargestellt (je nach Leistung des Speichers)... die muss doppelt so hoch und halb so breit sein. Das trifft dann auch auf die Einspeicherung in anderen Grafiken zu.**
...
: Der Anteil konventionell zu deckender Last entspricht dem Quotienten aus der summierten positiven Restlast und der summierten Netzlast bzw. aus der summierten positiven Restlast und der summierten Stromerzeugung.
: Der Anteil erneuerbarer Stromproduktion entspricht 100 Prozent - konventioneller Anteil.

:::

Der Anteil erneuerbarer Stromproduktion wird deshalb wie folgt berechnet:
...
Die Netzlast aus dem Datensatz verbrauch weicht mit 458 TWh geringfügig von der summierten Stromerzeugung aus dem Datensatz erzeugung in Höhe von 448 TWh ab. Dadurch unterscheiden sich die berechneten Anteile geringfügig voneinander.
...
**Hinweis?!: Abregelung erneuerbarer Erzeugung ist im Datensatz nicht erfasst, das waren im ersten Quartal 2023 immerhin 5,29 Prozent der erneuerbaren Erzeugung. <https://dserver.bundestag.de/btd/20/090/2009016.pdf> S. 2**
...
## Aufgabe analytische Datenanalyse
...
**Berechnen Sie entsprechend des von der Bundesregierung vorgegebenen Ausbaupfads den Anteil erneuerbarer Stromerzeugung für das Jahr 2030 und für das Jahr 2035 (siehe [@lst-zubaufaktoren]).**
  ...
  - Welcher Anteil erneuerbarer Stromerzeugung wird erreicht, wenn der Speicher halb so groß dimensioniert wird?
  
  - Als Netzlast verwenden Sie die Netzlast des Jahres 2023 (siehe untenstehenden Hinweis).
...
Der künftige Stromverbrauch wird wahrscheinlich nicht dem des Jahres 2023 entsprechen. Projektionen lassen einen Stromverbrauch von 658 TWh (Quelle 1) für das Jahr 2030 und von 670,4 TWh im Jahr 2034 (Quelle 2) erwarten. Die Steigerung des Stromverbrauchs gegenüber 2023 kann leicht berechnet werden:
...
**To Do: Quellennachweise**
Stromverbrauchprojektion 2030: <https://www.bmwk.de/Redaktion/DE/Downloads/E/prognos-bruttostromverbrauch-2018-2030.pdf?__blob=publicationFile&amp;v=2> S. 4 = 658 TWh
Stromverbrauchprojektion 2035: <https://www.netzausbau.de/SharedDocs/Downloads/DE/Bedarfsermittlung/2035/SR/Szenariorahmen_2035_Entwurf.pdf?__blob=publicationFile> S. 54 670,4 TWh (Szenario B)
:::

Das Bild kann das Titelbild für die Aufgabe werden :-) 
...
Witzig ist, dass hier die 80-20-Regel gilt. Mit 20 Prozent des Aufwands erreicht man 80 Prozent des Weges. Daten für 2035:

  - Anteil EE ohne Speicher: 0.89 --> Weg bis 100 Prozent = 0.11

  - mit relativer Speichergröße 0.20 wird ein EE-Anteil von 0.98 erreicht

  - dies entspricht 55.4 Pumpspeicheräquivalenten bzw. 2073.8 GWh, was 41.5 Jahresproduktionen der Tesla Gigafactory Berlin-Brandenburg in der ersten Ausbaustufe mit 50 GWh/a entspricht. Die Fabrik könne bis zu 250 GWh/a produzieren. Dann reichen 8,3 Jahresproduktionen für die Zielerreichung von 98 Prozent EE bis 2035. (<https://de.wikipedia.org/wiki/Liste_von_Batteriezellfabriken_in_Deutschlandhttps://de.wikipedia.org/wiki/Liste_von_Batteriezellfabriken_in_Deutschland>)
...
# Das Wichtigste
Datensätze kontrollieren, plausibilisieren und visualisieren --> das meiste erkennt man visuell, die Quantifizierung mit Kennzahlen ist eine wichtige Ergänzung (kann alleine aber irreführen [Beispiel: Abschaltung KKW]) 
...
Anteil EE 2030 und 2035 berechnen, wenn Biomasse in den Lastfolgebetrieb geht. **vorher kurz mal den Jahresgang Biomasse plotten.**
