...
**Steinkohle wird wie Erdgas gefahren --> Jahresdauerlinie Braun-, Steinkohle und Gas (übereinanderlegen?!).**  
**es wäre nett, Phasen des EE-Überschusses farblich zu hinterlegen (mit einem flotten Grau)**
...
Die Berechnung der Jahresvolllaststunden und die Visualisierung der Jahresgänge zeigen, dass es durch den hohen Anteil volatiler erneuerbarer Stromerzeugung im deutschen Stromsystem keine Grundlast mehr gibt, die von konventionellen Erzeugern bedient werden kann. Dies bedeutet, dass alle nicht erneuerbaren Erzeuger im Lastfolgebetrieb arbeiten. Auch der Bereich der Mittelllast wird mit zunehmendem Ausbau erneuerbarer Energien immer kleiner werden (siehe @tip-verdopplung-EE). (**ggf. ergänzen: für die verbleibende Spitzenlast werden Speicher mit Spitzenlasterzeugern konkurrieren. Anderswo habe ich das schön formuliert.**)

Dies kann am Beispiel der Steinkohle verdeutlicht werden, deren erzielte Jahresvolllaststunden näher an der klassischen Spitzenlasterzeugung aus Erdgas als an der Mittellasterzeugung durch Braunkohle liegt. Die Auslastung eines Stromerzeugers kann mit einer sortierten Jahresdauerlinie dargestellt werden.

::: {.callout-important}
## Definition sortierte Jahresdauerlinie

https://www.youtube.com/watch?v=rMxYJuGqR4s CC BY
Rohdaten nach Größe absteigend sortieren und als Balkendiagramm darstellen.
:::

**Eventuell ein HinweisDie Erstellung eines barplots mit 35.040 Balken dauert schlanke 49 Sekunden. Das "Problem" sind möglicherweise die xticks. Man muss vorher die Daten also ein bisschen aggregieren.
...
braunkohle_daily.sort_values(ascending = False).reset_index(drop = True).plot.line(lw = linienstärke, color = 'brown', alpha = 0.5, label = 'Braunkohle')

steinkohle_daily.sort_values(ascending = False).reset_index(drop = True).plot.line(lw = linienstärke, color = 'black', alpha = 0.5, label = 'Steinkohle')

erdgas_daily.sort_values(ascending = False).reset_index(drop = True).plot.line(lw = linienstärke, color = 'lightskyblue', alpha = 0.5, label = 'Erdgas')

plt.title(label = "sortierte Jahresdauerlinie für ausgewählte konventionelle Erzeuger")
plt.grid()
plt.legend()
plt.ylabel('durchschnittliche Stromerzeugung\n(auf Viertelstundenbasis in MWh)')
...
Beim Vergleich der erzielten Jahresvolllaststunden konnte festgestellt werden, dass Steinkohle anders als Braunkohle kein Mittelllasterzeuger mehr ist, sondern das Erzeugungsprofil eher der Spitzenlasterzeugung von Erdgas entspricht.

In der grafischen Darstellung der sortierten Jahresdauerlinie wird nun sogar deutlich, dass das Erzeugungsprofil der Steinkohle dem Erzeugungsprofil der Braunkohle sogar weniger ähnlich ist als das von Erdgas.
 
**Steinkohle ist das neue Erdgas**  
**Aber es war doch Gaskrise?!**

Das hätte man jetzt nicht unbedingt so erwartet, dass Erdgas viel mehr Leistung bringt, obwohl es weniger Jahresvolllaststunden hat. Die Grafiken hätte man so erwarten können, wenn man mit fotografischem Gedächnis die Ausgabe von .describe() verinnerlicht hätte. Aber weil Ablesen komplexer Informationen so schwierig ist, wird ja visualisiert. Die Jahresvolllaststunden von Erdgas sind so gering, weil die installierte Leistung ungefähr dem Doppelten der Steinkohle entspricht.

Das Restlastprofil kann mit einer Jahresdauerlinie visualisiert werden.
hierhier

To Do:  Wie ähnlich Erdgas und Steinkohle sind, visualisieren wir mit Jahresdauerlinien.


**Man könnte noch in jeden Graph die Restlast einzeichnen.**
...
Grenzstrom: 2 (3) Regime (EE-Überschuss, fossile Lastfolgebetrieb, Wenn Restlast kleiner gleich die Einspeisung in Pumpspeicher ist, kann der Anteil EE genau bestimmt werden.)
...
Was rein kommt, ist, was raus kommt. Dabei sind 3 Regime zu unterscheiden: a) EE-Überschuss, b) fossiler Lastfolgebetrieb, c) Restlast kleiner Verbrauch Pumpspeicher (= Mischung aus a) und b)). Der Grenzstrom ist vermutlich fast immer fossil (wenn gerade keine Überproduktion an EE vorliegt).  
**--> das gucken wir bei der explorativen Datenanalyse, in wie vielen Intervallen die Restlast negativ war**  
**Hinweis: ökologischer Fehlschluss möglich, da mit Salden für das Gesamtnetz gerechnet wird, die Regelzonen aber kleinteiliger organisiert sind und die physischen Stromflüsse unbekannt sind.**
