...
Wenn alle Dateien korrekt eingelesen wurden, können Arbeitsdateien mit dem Befehl `df.copy()` angelegt werden. Mit diesen wird im folgenden gearbeitet. Dadurch bleiben die Rohdaten immer verfügbar und können bei Bedarf, beispielsweise nach einem versehentlichen Überschreiben der Arbeitsdateien, erneut geladen werden.
...
Die beschreibende Statistik der Daten offenbart beispielsweise, dass Onshore Wind den größten Beitrag zur Stromerzeugung lieferte. Darüber hinaus ist auffällig, dass weder Onshore Wind noch Photovoltaik eine minimale Erzeugung von 0 aufweisen, was jedoch für Wind Offshore und Kernenergie der Fall ist.  
**Warum ist das bei Photovoltaik so? -- Simone fragt nach** 
...
Einen schnellen Überblick beispielsweise über die Verteilung der Gesamterzeugungsleistung nach Erzeugungsart verschafft ein Kreis- bzw. Ringdiagramm. Dieser Diagrammtyp hat, wie im **Methodenbaustein Grundlagen der Statistik (Kapitel 2 einzelne Merkmale)** erläutert, allerdings den Nachteil, dass dieser mit steigender Anzahl von Merkmalsausprägungen schnell unübersichtlich wird und Winkel kaum exakt abgelesen werden können.
...
:::{#tip-Ringdiagramm .callout-tip collapse="true" title="Lösung Zeichenkette"}
...
To Do:
  ...
  * Hinweis in Aufklapper. pie-chart with exploded bar erstellen. Aus der matplotlib Dokumentation:  
...
# use 4 out of 8 panels = middle 2 rows
plt.subplot(nrows, ncols, (3, 6))
plt.pie(x = plotting_data.sum(), colors = plt.cm.Pastel1.colors, startangle = 350, labels = list(plotting_data.columns), autopct='%1.1f%%', pctdistance = 0.7, textprops = dict(size = font_size +2 ))
...
To Do:
  ...
  * Grafiktitel, x-Achsenbeschriftung für Barplots
...
:::
summe über alle Erzeugungstypen und Visualisierung in einem Tortendiagramm
--> das ist aber nicht wirklich übersichtlich --> wir gruppieren die Erneuerbaren und fügen eine eigene Legende für diese ein 

Daten beschreiben und visualisieren --> Erst mal mit installierter Leistung, das ist am einfachsten.


To Do:

  * [MWh] im Kuchendiagramm unterdrücken.

  * Farben im Kuchendiagramm und Balkendiagramm angleichen (Kuchenstück Erneuerbare gleiche Farbe wie Balken Erneuerbare)
Erzeugung im Jahresverlauf plotten --> Abschaltung der AKW erkennen.

Datensätze verstehen, ein Gefühl dafür bekommen, mögliche Fehler im Datensatz erkennen. Vorbereitung der schließenden Datenanalyse (machen wir das überhaupt)

gestapeltes Histogramm für die Erzeugung in einem Monat oder in einer Woche - eine Woche im Winter, eine Woche im Sommer
