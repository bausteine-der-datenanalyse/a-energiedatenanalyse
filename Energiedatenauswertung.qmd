...
Wirkungsgrad
: Einspeicherwirkungsgrad vermindert den verfügbaren Überschussstrom. Ist die Summe der erneuerbaren Überschussproduktion kleiner als die aus dem Speicher zu bedienende Restlast, kann der Speicher um den Einspeicherwirkungsgrad kleiner dimensioniert werden.
: Ausspeicherwirkungsgrad (inklusive Speicherverluste) erhöht die durch den Speicher zu bedienende Last. Ist die Summe der erneuerbaren Überschussproduktion größer als die aus dem Speicher zu bedienende Restlast, muss der Speicher um den Ausspeicherwirkungsgrad größer dimensioniert werden.
...
**Dazu muss auf eine Listenoperation zurückgegriffen werden, die zum Glück gar nicht so langsam ist. Die Berechnung der Speichergröße und des Jahresgänge in Jahr Null und den Folgejahren hat 0.09 Sekunden gedauert. Ich kann bei Bedarf auch die Probleme mit NumPy / Pandas erklären.**.
Auf dieser Grundlage kann eine Funktion entwickelt werden, um anhand der Restlast die erforderliche Speichergröße zu berechnen. Um die Größenordnung besser einordnen zu können, wird die Speichergröße zur bestehenden Pumpspeicherkapazität von 37,4 GWh ins Verhältnis gesetzt. Die Restlast wird wie folgt bestimmt:
...
:::{#tip-Wirkungsgrad .callout-tip collapse="true" title="Musterlösung erneuerbarer Überschuss oder Defizit mit Wirkungsgrad"}
...
### Berechnung der Speichergröße ohne Wirkungsgrad
```{python}
# Speichergröße berechnen
...
print(f"erforderliche Speichergröße 2023: {speicher_2023[0]:.1f} MWh\nDies entspricht {speicher_2023[0] / pumpspeicherkapazität_MWh:.1f} Pumpspeicheräquivalenten.")

```

Der Füllstand des Speichers im Jahresgang kann grafisch dargestellt werden. Die Funktion gibt ein Tupel zurück. An Position 0 ist die Speichergröße gespeichert, an Position 1 der Jahresgang in Jahr 1, an Position 2 der Jahresgang in Jahr 2. Da die Funktion die freie Kapazität als Liste zurückgibt, muss der Füllstand des Speichers mit `speichergröße - pd.Series(data = jahresgang_speicher)` berechnet werden. **TO DO: panel-tabset, fig-alt, fig-cap ergänzen**

``` {python}
#| echo: false

# Daten erzeugen
jahresgang_speicher_jahr1 = pd.Series()
jahresgang_speicher_jahr1 = speicher_2023[0] - pd.Series(speicher_2023[1])

jahresgang_speicher_jahr2 = pd.Series()
jahresgang_speicher_jahr2 = speicher_2023[0] - pd.Series(speicher_2023[2])
...
Die benötigte Kapazität des Speichers ergibt sich aus den ereuerbaren Erzeugungsüberschüssen während der Weihnachtsfeiertage. Diese Kapazitäten bleiben im Rest des des Jahres ungenutzt. Der Speicher ist die meiste Zeit des Jahres leer. Der Jahresgang für das erste und das zweite Jahr ist identisch.

### Berechnung der Speichergröße mit Wirkungsgrad
...
def berechne_speichergröße(data, einspeicherwirkungsgrad = 1, ausspeicherwirkungsgrad = 1, output = False):
  ...
  wirkungsgrad = einspeicherwirkungsgrad * ausspeicherwirkungsgrad
  data_x2 = pd.concat([data, data])

  überschuss = prüfe_EE_Überschuss(data, wirkungsgrad)
  ...
  if output: # output = True
    ...
    print(f"\n\nÜberschuss: {überschuss}\t Einspeicherwirkungsgrad: {einspeicherwirkungsgrad}\t Ausspeicherwirkungsgrad: {ausspeicherwirkungsgrad}")
    print(f"Summe data: {data.sum()}\nSpeichergröße: {speichergröße}")
    print(dataset)

  else: # output = False
    return speichergröße, jahresgang_speicher[ : len(jahresgang_speicher) // 2 ], jahresgang_speicher[ len(jahresgang_speicher) // 2 : ]
    
speicher_2023 = berechne_speichergröße(restlast, einspeicherwirkungsgrad = 0.9, ausspeicherwirkungsgrad = 0.88, output = False)
...
### Dies und das
Wirkungsgrad Pumpspeicher in Prozent: 79.10 ... also sagen wir 
**Die Aufteilung lässt sich anhand der Zyklenzahl gut begründen (die kommt aber erst später)** Da die Datenreihe sehr lang ist, bietet es sich an, für die Funktionsentwicklung eine kürzere Reihe von Testdaten zu benutzen. Auf diese Weise werden Berechnungen schneller durchgeführt und Zwischenergebnisse und Ergebnisse können bei Bedarf zur Überprüfung ausgegeben werden. Die folgende Funktion simuliert mit zehn Werten eine Restlastkurve, die im Jahresgang zufällig eine erneuerbare Überschussproduktion bzw. eine positive Restlast darstellt. Durch Anpassen der Werte für a und b kann das Ergebnis gesteuert werden. **Eventl. Ausgabefunktion hier ergänzen**
