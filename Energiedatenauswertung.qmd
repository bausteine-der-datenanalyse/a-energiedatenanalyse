...
**wird `my_axis = plt.gca()` überhaupt benötigt?** 
### Daten aggregieren
...
Im Folgenden werden verschiedene **Anzahl benennen** Grundprinzipien der Speicherauslegung und die entsprechenden Verfahren zu ihrer Berechnung entwickelt. Grundlage aller Überlegungen ist die Restlastkurve. Werte kleiner als Null entsprechen einer erneuerbaren Überschussproduktion, Werte größer als Null einer durch Lastfolgekraftwerke oder Speicher zu bedienenden Netzlast. Die Restlast wird wie folgt bestimmt:  
**Dazu muss auf eine Listenoperation zurückgegriffen werden. Ich kann bei Bedarf auch die Probleme mit NumPy / Pandas erklären (.clip(0) maskiert nur die Ausgabe).**
...
Die erforderliche Größe des Stromspeichers ist einerseits abhängig von dem Verhältnis aus der Menge der erneuerbaren Überschussproduktion und der aus dem Speicher zu bedienenden Restlast. Andererseits spielen die Wirkungsgrade des Speichers beim Ein- und Ausspeichern von Strom eine Rolle.
...
plt.fill_between(x = np.linspace(365 // 2, 365, 365 // 2), y1 = np.linspace(0.5, 0.5, 365 // 2), facecolor = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
...
plt.fill_between(x = np.linspace(365 // 2, 365, 365 // 2), y1 = np.linspace(0.64, 0.64, 365 // 2), facecolor = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
...
plt.fill_between(x = np.linspace(365, 365 // 2 + 365, 365 // 2), y1 = np.linspace(0.5, 0.5, 365 // 2), facecolor = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
...
plt.fill_between(x = np.linspace(365, 365 + 365 // 2, 365 // 2), y1 = np.linspace(0.5, 0.5, 365 // 2), facecolor = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
...
plt.fill_between(x = np.linspace(365 // 2, 365, 365 // 2), y1 = np.linspace(1, 1, 365 // 2), facecolor = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
...
plt.fill_between(x = np.linspace(365 // 4, 365 // 2, 365 // 4), y1 = np.linspace(1, 1, 365 // 4), color = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
plt.fill_between(x = np.linspace((365 * 3) // 4, 365, 365 // 4), y1 = np.linspace(1, 1, 365 // 4), color = 'None', edgecolor = my_color, hatch = r'\\')
...
plt.fill_between(x = np.linspace(365 // 4, 365 // 2, 365 // 4), y1 = np.linspace(0.25, 0.25, 365 // 4),facecolor = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
plt.fill_between(x = np.linspace((365 * 3) // 4, 365, 365 // 4), y1 = np.linspace(0.75, 0.75, 365 // 4), facecolor = 'None', edgecolor = my_color, hatch = r'\\')
...
plt.fill_between(x = np.linspace(365 // 4, 365 // 2, 365 // 4), y1 = np.linspace(0.25, 0.25, 365 // 4), facecolor = 'None', edgecolor = my_color, hatch = r'\\', label = 'Ausspeicherung')
plt.fill_between(x = np.linspace((365 * 3) // 4, 365, 365 // 4), y1 = np.linspace(1, 1, 365 // 4), facecolor = 'None', edgecolor = my_color, hatch = r'\\')
...
speicher_2023 = berechne_speichergröße(restlast, einspeicherwirkungsgrad = 0.8, ausspeicherwirkungsgrad = 0.8,output = False)

print(f"erforderliche Speichergröße 2023: {speicher_2023:.1f} MWh\nDies entspricht {speicher_2023 / pumpspeicherkapazität_MWh:.1f} Pumpspeicheräquivalenten.")   
```

**ist hier ersichtlich, welche Wirkungsgrade benutzt werden? Ich könnte das noch bei output hinterlegen und die Funktion zwei mal aufrufen.**
:::

#### Jahresgang des Speichers berechnen
Nachdem die Speichergröße bestimmt wurde, kann der Jahresgang des Speichers für das erste und das zweite Jahr bestimmt werden. Der Jahresgang des Speichers für das zweite Jahr ist das stabile Gleichgewicht, mit dem eine gegebene Restlastkurve beliebig oft durchlaufen werden kann. Die Funktion gibt, wenn `output = False` ist, ein Tupel zurück. An Position 0 ist die Speichergröße gespeichert, an Position 1 der Jahresgang in Jahr 1 als pd.Series, an Position 2 der Jahresgang in Jahr 2 als pd.Series. Der Füllstand des Speichers im Jahresgang kann grafisch dargestellt werden.
...
def zyklenzahl_berechnen(data, output = False):
  ...
  # Jahresgang Jahr 1 eine Null voranstellen, Jahresgang Jahr 2 den letzen Wert aus Jahr 1
  jahresgang_speicher_jahr1 = pd.concat([pd.Series(0), jahresgang_speicher_jahr1])
  jahresgang_speicher_jahr2 = pd.concat([jahresgang_speicher_jahr1[ len(jahresgang_speicher_jahr1) - 1: ], jahresgang_speicher_jahr2])
...
Wenn der Wirkungsgrad berücksichtigt wird, steigt die Zyklenzahl, weil der Speicher kleiner dimensioniert ist.

**Stellen Sie den Jahresgang der Pumpspeicherkraftwerke dar. Welche Zyklenzahl erreichen die Pumpspeicherkraftwerke?**  
Hierfür muss man die Datensätze erzeugung und verbrauch zusammenlegen, wobei die Verbrauchswerte negativ gesetzt werden müssen (dass entspricht dann dem EE-Überschuss). Die Speichergröße ist im Objekt `pumpspeicherkapazität_MWh = 37.4 * 1000` gespeichert. **Hier müsste man vorher gucken, ob es in den Datensätzen erzeugung und verbrauch gleichzeitig auftretende Werte > 0 gibt. Die sollten etwa 297 sein (Ausspeicherung von 11,1 TWh / 37,4 GWh Kapazität). Die Fahren ja nicht im EE-Betrieb**

### Kappung
**Man könnte noch eine gestrichelte Linie einzeichnen, die den Gang der Speicherfüllung zeigt. Dafür habe ich auch schon eine Funktion.**  
**TO DO: Grafiken aktualisieren, fig-alt**
...
#### Berechnung
**To Do: Variante ohne Wirkungsgrad zusammenkürzen.**
für die gleitende Differenzberechnung muss dem Jahresgang in Jahr 1 eine Null vorangestellt werden, dem Jahresgang in Jahr 2 der letzte Wert aus Jahr 1
jahresgang_speicher_jahr1.diff().clip(lower = 0).sum() macht die Reihe um eins kürzer
...
## Ausgabe: Wenn output = False werden für jede Speichergröße die gekappten Strommengen in Jahr 1 und Jahr 2 ausgegeben.
## Ausgabe: Wenn output = True werden zusätzlich die Werte der Basiskurve data und für alle Jahre die eingespeicherte Energiemenge und realisierte Zyklenzahl in Jahr 1 und Jahr 2 ausgegeben
...
effekt_kappung_berechnen(restlast, einspeicherwirkungsgrad = 1, ausspeicherwirkungsgrad = 1, output = True)
```

:::

### Anteil EE bestimmen
Ausbauziele durchrechnen: welcher Anteil EE wird erreicht.  
Die benötigte Kapazität des Speichers sinkt, wenn Ein- und Ausspeicherwirkgungsgrade berücksichtigt werden. Für den Gesamtwirkungsgrad können die bestehenden Pumpspeicherkraftwerke als Orientierung dienen. Der Gesamtwirkungsgrad wurde mit 79.10 Prozent berechnet. Diesem Wirkungsgrad entspricht näherungsweise eine Aufteilung von Einspeicherwirkungsgrad = 0.9, Ausspeicherwirkungsgrad = 0.88.

ein Test: die Summe der positiven Restlast müsste dem Anteil nicht-EE entsprechen --> haut so ungefähr hin (ein Prozent), müsste passen, wenn man mit summe erzeugung rechnet
...
print(f"Summe positive Restlast [MWh]: {restlast.clip(lower = 0).sum()}\n"
      ...
      f"Quotient Netzlast: {restlast.clip(lower = 0).sum() / verbrauch['Gesamt (Netzlast) [MWh]'].sum():.2f}\n\n"
      f"Summe Erzeugung: {erzeugung.sum(numeric_only = True).sum()}\n"
      f"Quotient Erzeugung: {restlast.clip(lower = 0).sum() / erzeugung.sum(numeric_only = True).sum():.2f}")
```

**hier kommt dann noch eine Code-Zelle**
...
Man könnte das aber auch aus dem Jahresgang des Ladestands abgreifen. Der Speicher wird geladen... man sucht elementweise die positive Differenz (der Speicher wird voller) und summiert die. Dann gleicht man das ab mit dem gekappten Speicher -- da müsste die Summe kleiner sein und die Differenz beider Summen ist der nicht eingespeicherte Strom.
Man muss das für Jahr 1 und Jahr 2 berechnen (Jahr 2 ist relevanter)

dem Jahresgang muss eine Null vorangestellt werden, falls es in der ersten Periode zu einer Einspeicherung kommt
data.diff().clip(lower = 0).sum()

dann für originale und verschiedene Speichergrößen vergleichen

Es wird ja schon jetzt gekappt, wenn EE-Überschuss herrscht. Das kann man bei EE-Überschuss in der Formel selbst abgreifen und zwar hier:
...
man muss die Differenz aus i und freie_speicherkapazität bilden - das ist der "Überhang" der gekappt wird
    else: # i <= 0 EE-Überschuss
      
      # wenn der Speicher voll ist, muss Überschuss verworfen werden.
      if freie_speicherkapazität + i * einspeicherwirkungsgrad < 0:
        gekappte_einspeisung = i - freie_speicherkapazität 
        freie_speicherkapazität = 0

Man könnte das aber auch aus dem Jahresgang des Ladestands abgreifen. Der Speicher wird geladen... man sucht elementweise die positive Differenz (der Speicher wird voller) und summiert die. Dann gleicht man das ab mit dem gekappten Speicher -- da müsste die Summe kleiner sein und die Differenz beider Summen ist der nicht eingespeicherte Strom.
Man muss das in Jahr 2 abgreifen
