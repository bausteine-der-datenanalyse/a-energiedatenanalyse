...
Die Ausbauziele der Bundesregierung legen also für das Jahr 2030 eine installierte Leistung fest in Höhe von:
...
print(f"Wind an Land:\t{( zubaufaktor_windonshore := 115 / (installierte_leistung['Wind Onshore [MW]'].sum() / 1000) ):.2f}\n"
      f"Wind auf See:\t{( zubaufaktor_windoffshore := 30 / (installierte_leistung['Wind Offshore [MW]'].sum() / 1000) ):.2f}\n"
      f"Solar:\t\t\t{( zubaufaktor_solar := 215 / (installierte_leistung['Photovoltaik [MW]'].sum() / 1000) ):.2f}\n"
      f"Biomasse:\t\t{8.4 / (installierte_leistung['Biomasse [MW]'].sum() / 1000):.2f}")

```

Das bedeutet, dass die Produktion durch Windkraftanlagen an Land verdoppelt sowie durch Windkraftanlagen auf See und durch Photovoltaik verdreieinhalbfacht werden soll. Biomasse soll nicht weiter ausgebaut werden.

## Hintergrund: Speicherauslegung
Stromspeicher speichern die Stromerzeugung in Überschussphasen, um die Energie in Phasen der Unterdeckung wieder abzugeben. Die Auslegung 

Zyklenzahl usw.

Ein paar anschauliche Grafiken zur Speicherung. Wichtig ist auch die Verteilung

**kann man erst fill_between und dann plt.plot zeichnen? Dann wären die Randlinien auf jeden Fall richtig.**
...
plt.fill_between(x = np.linspace(365 // 2, 365, 365 // 2), y1 = np.linspace(0.5, 0.5, 365 // 2), hatch = '//', facecolor = 'white', edgecolor = my_color, label = my_label)
...
Vorzeichen der summierten Restlast (bessere Bezeichnung gesucht)
...
: Ist die Summe der erneuerbaren Überschussproduktion kleiner als die Summe der erneuerbaren Unterproduktion, entspricht die erforderliche Speichergröße der Summe der erneuerbaren Überschussproduktion.
: Ist die Summe der erneuerbaren Überschussproduktion größer als die Summe der erneuerbaren Unterproduktion, entspricht die erforderliche Speichergröße der Summe der erneuerbaren Unterproduktion.
...
Anzahl Vollzyklen
: Die erforderliche Größe des Stromspeichers ist abhängig von der Verteilung der Restlast und der damit realisierten Anzahl Vollzyklen. Ein Vollzyklus bedeutet, dass ein Speicher vollständig geladen und wieder entladen wird.
...
### plot only the hatch
plt.fill_between(x = np.linspace(365 // 2, (365 * 3) // 4, 365 // 4), y2 = np.linspace(-0.25, -0.25, 365 // 4), y1 = np.linspace(-0.5, -0.5, 365 // 4),  facecolor = 'white', edgecolor = my_color, hatch = '//')
### plot only the fill
...
### positive Restlast
plt.fill_between(x = np.linspace((365 * 3) // 4, 365, 365 // 4), y1 = np.linspace(1, 1, 365 // 4), hatch = '//', facecolor = 'white', edgecolor = my_color, label = my_label + ' = 1')
...
plt.tight_layout()
# plt.fill_between(x = np.linspace(365 // 4, 365 // 2, 365 // 4), y1 = np.linspace(0.25, 0.25, 365 // 4), hatch = '//', facecolor = 'white', edgecolor = my_color) 
```

Teilzyklen
: Erklärung Teilzyklen
: Ist die Summe der erneuerbaren Überschussproduktion kleiner als die Summe der erneuerbaren Unterproduktion, entspricht die erforderliche Speichergröße dem Minimum der kummulierten Summe der Restlast '.cumsum', wenn diese niemals größer 0 gesetzt wird (das Überschreiten wäre der Moment, in dem der Speicher leer ist).  
: 'Restlast.sum() > 0 = .cumsum Restlast (aber nie kleiner 0)' **Ich glaube nicht, dass man die Erklärung so versteht.**
: Ist die Summe der erneuerbaren Überschussproduktion größer als die Summe der erneuerbaren Unterproduktion, entspricht die erforderliche Speichergröße dem Maximum der kummulierten Summe der Restlast'.cumsum', wenn diese niemals kleiner 0 gesetzt wird (das Unterschreiten wäre der Moment, in dem der Speicher überdimensioniert ist).  **Ich glaube nicht, dass man die Erklärung so versteht.**
...
### plot the hatch
plt.fill_between(x = np.linspace(1, 365 // 4, 365 // 4), y1 = np.linspace(-0.5, -0.5, 365 // 4), facecolor = 'white', edgecolor = my_color, hatch = '//', label = my_label + ' mit Kappung = 0.5')
...
:::
  ...
  Diese könnten, wenn Sie gespeichert werden, helfen, die Phasen mit positiver Restlast ebenfalls aus erneuerbaren Energien zu decken.
...
man könnte zeigen: Restlast, Restlast ohne Speicher (alle negativen Werte = 0), Restlast mit optimalen Speicher (alle negativen Werte summieren sich und tragen darauf folgende positive Werte ab)
schlechtester Fall, halber Zyklus = halber Speicherbedarf.
Wirkungsgrad ist schwierig: bei der Auslegungsgröße sind eigentlich nur Einspeicherverluste relevant (um die vermindert sich die benötigte Größe des Speichers)
